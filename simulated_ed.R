...
cat("ZCTAs:", length(unique(ed_combined$GEOID)), "\n")
cat("Total rows:", nrow(ed_combined), "\n")
cat("Total ED events:", sum(ed_combined$hri_ed), "\n")
cat("ZCTA-days with ≥1 ED event:", sum(ed_combined$hri_ed > 0), "\n")

# overall simulated rate vs. target real_ed_rates (9.5-19.6 per 10,000/yr)
zcta_summary <- ed_combined %>%
  group_by(GEOID, pop) %>%
  summarize(total_events = sum(hri_ed), .groups = "drop")
cat("Mean simulated annual rate per 10,000:", 
    mean(zcta_summary$total_events / zcta_summary$pop * 10000 / 11), "\n")