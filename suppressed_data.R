...
length(extra_in_daily) # extra zips in daily file not in clusters
print(missing_from_daily)

# =========================================================================
# IMPUTE SUPPRESSED CELLS (<12), BOUNDED 1-11, DAILY D2 only
# =========================================================================
...
allocate_remainder <- function(pool, share, baseline) {
  raw_alloc <- pool * share # each day's decimal share of the pool
...
daily_ed <- daily_ed %>% 
  ...
  group_by(zip, month) %>% mutate(
    ...
    zero_month_pick = sample.int(n(), 1),
    ...
    D2 = case_when(!is.na(mo_num) ~ D2_exact_case,
                   ...
                   mo_suppressed & !exceeds_cap & sum(D2) == 0 ~ 
                     if_else(row_number() == zero_month_pick, 1, D2), 
  ...
  select(-mo_suppressed, -mo_upper, -mo_num, -baseline, -share,
         ...
         -exceeds_cap, -pool_cap, -D2_cap_case, -zero_month_pick)
gc()
